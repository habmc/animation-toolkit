...
void CurveEditor::scene()
{
  ...
  for (float j = 0.0f; j < mSpline.getDuration(); j += 0.01f)
  {
    setColor(vec3(0, 0, 1));
    drawSphere(mSpline.getValue(j), 1);
  }
  if (mShowControlPoints)
  {
    if (mSpline.getInterpolationType() == "Catmull-Rom")
    {
      for (int i = 1; i < mSpline.getNumControlPoints() - 2; i += 3)
      {
        ...
        drawLine(mSpline.getControlPoint(i + 1), mSpline.getControlPoint(i));
        drawLine(mSpline.getControlPoint(i + 2), mSpline.getControlPoint(i + 1));
      }
    }
    else if (mSpline.getInterpolationType() == "Hermite")
    {
      ...
      {
        vec3 point = mSpline.getControlPoint(i - 1) + mSpline.getControlPoint(i);
        setColor(vec3(1, 1, 0));
        drawSphere(point, 5);
        drawLine(point, mSpline.getControlPoint(i - 1));
      }
    }
  }
}
